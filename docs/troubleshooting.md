...
### Wi-Fi shows "portal blocked" / alerts stopped at a marina
- The marina network has a captive portal that the device cannot sign in to itself. Open the Wi-Fi page in CONFIG mode: either set a **Custom MAC address** to one already authenticated on the network, or ask the network admin to whitelist the device's current MAC (shown on the same card). See Configuration → Captive portals.
...
- The device re-checks every 2 minutes, so once the MAC is authenticated or whitelisted the banner clears and alerts resume automatically.
...
- If the Wi-Fi page shows **"portal blocked"**, a captive portal is blocking outbound traffic — authenticate/whitelist the device's MAC first (see above).
